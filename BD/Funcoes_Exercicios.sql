...
CREATE OR REPLACE FUNCTION SALARIOEMPREGADO (P_CPF INT) RETURNS FLOAT AS $$
DECLARE 
    V_SALARIO FLOAT;
BEGIN
    BEGIN
        SELECT SALARIO INTO V_SALARIO FROM EMPREGADO WHERE CPF = P_CPF;
       --IF SALARIO IS NULL THEN
         --RAISE EXCEPTION 'CPF NÃO ENCONTRADO';
        --END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION 'ERRO AO BUSCAR O SALÁRIO';
    END;
    RETURN V_SALARIO;
END;
$$ LANGUAGE PLPGSQL;
